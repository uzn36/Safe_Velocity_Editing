...
def encode_prompt_Qwen(
    ...
    prompt_embeds = _encode_prompt_with_Qwen(
        text_encoder=text_encoders[1],
        tokenizer=tokenizers[1],
        max_sequence_length=max_sequence_length,
        prompt=prompt,
...
def collate_fn_t2i(examples):
    target_image = torch.stack([example["images"] for example in examples])
    ...
    empty_captions = [example["empty_captions"] for example in examples]
...
class FluxPipeline_sCM(DiffusionPipeline, FluxLoraLoaderMixin):
    r"""
    ...
    _callback_tensor_inputs = ["latents", "prompt_embeds", "noise_pred", "denoised"]
    ...
    def __call__(
        ...
        flops_total_dyn, flops_total_stat = 0, 0
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                scm_timestep_expanded = scm_timestep.view(-1, 1, 1)
                ...
                # True CFG: second forward with negative prompt, then interpolate
                if do_true_cfg:
...
class FluxPipeline_MeanFlow(DiffusionPipeline, FluxLoraLoaderMixin):
    r"""
    The Flux pipeline for text-to-image generation.
    ...
    def __init__(
        ...
        text_encoder_2: T5EncoderModel,
        tokenizer_2: T5TokenizerFast,
        transformer: FluxTransformer2DModelDualTimestepTimestepX1GuidanceX1,
    ):
        super().__init__()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < len(timesteps) - 1:
                    ...
                    r_timestep = next_t.expand(latents.shape[0]).to(latents.dtype)
                else:
                    r_timestep = torch.zeros_like(timestep)

                # second_time_input = r_timestep / 1000.0

                noise_pred = self.transformer(
                    hidden_states=latents,
                    # YiYi notes: divide it by 1000 for now because we scale it by 1000 in the transforme rmodel (we should not keep it but I want to keep the inputs same for the model for testing)
                    timestep=timestep.flatten() / 1000,
                    timestep2=(timestep-r_timestep).flatten() / 1000.0,
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                    encoder_hidden_states=prompt_embeds,
                    txt_ids=text_ids,
